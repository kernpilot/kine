// kine watch-churn load generator, v2 — open-loop arrivals and a skewed key
// distribution.
//
// TWO METHODOLOGICAL FIXES over loadgen/. Both were prompted by a published
// PostgreSQL write benchmark that measured a real saturation ceiling and named
// the traps that hide it.
//
//  1. COORDINATED OMISSION. v1 is closed-loop: a writer sends, waits for the
//     reply, then sends again. When the server slows down the driver slows down
//     with it, so the offered load silently drops and the recorded latency is
//     only service time — the queueing delay a real client would have seen never
//     appears. v2 measures every operation from its SCHEDULED start, so a
//     request that could not be issued on time carries that delay in its number.
//     Expect worse percentiles than v1. That is the point: v1's were flattering.
//
//  2. UNIFORM KEYS ARE WRONG FOR KUBERNETES. v1 cycles keys evenly. A real
//     control plane is violently skewed — leader-election Leases are rewritten
//     every couple of seconds, node status and endpoints churn constantly, and a
//     handful of objects absorb a large share of all writes. Uniform access
//     understates row and page contention on exactly the hot objects that decide
//     whether a dense multi-tenant plane holds up. v2 supports a Zipfian
//     distribution over the keyspace.
//
...
type result struct {
	...
	SchedP99Ms       float64  `json:"write_scheduled_p99_ms"`
	SchedP999Ms      float64  `json:"write_scheduled_p999_ms"`
	MaxSchedLagMs    float64  `json:"max_schedule_lag_ms"`
...
func main() {
	var (
		...
		zipf          = flag.Float64("zipf", 0, "Zipfian skew s (>1 skews; 0 = uniform, v1 behaviour)")
		warmup        = flag.Duration("warmup", 0, "discard measurements for this long after load starts")
	...
	for w := 0; w < *writers; w++ {
		...
		go func(w int, c *clientv3.Client) {
			...
			// OPEN-LOOP SCHEDULE. Arrivals are fixed in advance from the rate,
			// so a slow server cannot slow the arrival stream down. Each
			// operation's latency is measured from when it was DUE, not from
			// when the driver got around to it, so time spent waiting behind a
			// backlog is counted instead of silently discarded.
			var interval time.Duration
			if *rateCap > 0 {
				interval = time.Second / time.Duration(*rateCap)
			}
			// Per-writer Zipfian generator over the keyspace. Kubernetes access
			// is heavily skewed; uniform keys understate contention on the hot
			// objects (Leases, node status, endpoints) that decide whether a
			// dense plane holds up.
			var zg *rand.Zipf
			if *zipf > 1.0 && *keyspace > 1 {
				zg = rand.NewZipf(rand.New(rand.NewSource(int64(w)+1)), *zipf, 1.0, uint64(*keyspace-1))
			}
			measureFrom := started.Add(*warmup)
			...
			for time.Now().Before(deadline) {
				// due = when this operation SHOULD have started.
				due := time.Now()
				if interval > 0 {
					due = started.Add(time.Duration(n) * interval)
					if d := time.Until(due); d > 0 {
						time.Sleep(d)
					}
					// No catch-up sleep when already late: falling behind is the
					// signal, and sleeping it away is exactly the omission this
				...
				key := fmt.Sprintf("/registry/bench/w%d/w%dkey%d", w%8, w, idx)
				...
				t0 := time.Now()
				lag := t0.Sub(due) // how late the driver was issuing this
				resp, err := c.Txn(ctx).
				...
				if err != nil {
					...
					if t0.After(measureFrom) {
						...
						// Scheduled latency = queueing delay + service time.
						schedLat = append(schedLat, ms+float64(lag.Microseconds())/1000.0)
						if l := float64(lag.Microseconds()) / 1000.0; l > maxLag {
							maxLag = l
						}
						latMu.Unlock()
					}
	...
	r := result{
		...
		SchedP99Ms:       pct(schedLat, 0.99),
		SchedP999Ms:      pct(schedLat, 0.999),
		MaxSchedLagMs:    maxLag,
